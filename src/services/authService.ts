...
const authServices = {
    signUp: async (
        ...
        if (
            ...
            new Date().valueOf() - existingUser.lastOtpRequest < coolDownPeriod
        ) {
            throw new Error(
        ...
        await transporter.sendMail({
            from: 'Marites reypjbaliguat@gmail.com',
            to: email,
    ...
    googleLogin: async (_: any, { token }: any) => {
    ...
    verifyOTP: async (_: any, { email, otpCode }: any) => {
        const user = await prisma.user.findUnique({ where: { email } });
    ...
    resendOTP: async (_: any, { email }: any) => {
        ...
        if (
            user.lastOtpRequest &&
            new Date().valueOf() - user.lastOtpRequest < coolDownPeriod
        ) {
            throw new Error(
